...
open (my $OS, ">:utf8", "$per.source.html") or die "Could not write src file\n";
open (my $OM, ">:utf8", "$per.mt.html") or die "Could not write mt file\n";
open (my $OP, ">:utf8", "$per.pe.html") or die "Could not write pe file\n";
open (my $OSUM, ">:utf8", "$per.summary") or die "Could not write summary file\n";
open (my $OHIS, ">:utf8", "$per.history.html") or die "Could not write history file\n";
open (my $OOPS, ">:utf8", "$per.operations.html") or die "Could not write history (operations) file\n";
...
my @header = qw/id type status producer revisions editing_time assessing_time src_tokens mt_tokens pe_tokens src_len mt_len pe_len keystrokes white_keys control_keys alphanum_keys unchanged lc_unchanged secondsPerWord oer-word oer-len oer-time irate drate/;
...
foreach my $xmlTask (@xmlTasks){
	...
	my $output = {
		...
		'white_keys' => $revision->{"$INDICATOR:white-keystyped"},
		'control_keys' => $revision->{"$INDICATOR:iso-keystyped"},
		'alphanum_keys' => $revision->{"$INDICATOR:nonwhite-keystyped"},
